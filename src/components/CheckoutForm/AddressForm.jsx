...
const AddressForm = ({ checkoutToken, next }) => {
    ...
    // make an array of country arrays so they can be looped over and grab the 'code name' for selected country
    const countries = Object.entries(shippingCountries).map(([code, name]) => ({ id: code, label: name }));
    // as above for subdivisions but for county/state etc
    const subdivisions = Object.entries(shippingSubdivisions).map(([code, name]) => ({ id: code, label: name }));
    // already an array so mapping shipping options. gets price for specific shipping (overseas etc)
